...
if not defined TOOLCHAIN_DIR (
    for /d %%D in ("C:\ST\STM32CubeIDE_*\STM32CubeIDE\plugins\com.st.stm32cube.ide.mcu.externaltools.gnu-tools-for-stm32.*") do set "TOOLCHAIN_DIR=%%D\tools\bin"
)
...
set "GDB_PORT=2333"             REM J-Link GDB Server TCP 端口
set "OPENOCD_IF=stlink.cfg"     REM OpenOCD 调试器配置:stlink.cfg / cmsis-dap.cfg 等
set "UART_PORT=COM5"            REM 调试串口(默认值,uart_capture.py 可用 --port 覆盖)
...
echo [paths] device=%JLINK_DEVICE% if=%JLINK_IF% speed=%JLINK_SPEED%kHz gdb_port=%GDB_PORT% uart=%UART_PORT%@%UART_BAUD%
exit /b 0
